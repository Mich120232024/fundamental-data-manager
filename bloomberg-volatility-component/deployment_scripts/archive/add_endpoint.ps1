# PowerShell script to add Bloomberg reference endpoint
# This will be executed on the VM

# Read the current file
$content = Get-Content 'C:\BloombergAPI\main.py' -Raw

# Define the new endpoint code to add
$newEndpointCode = @'
...
# Find the position after the FX rates endpoint (after line containing "raise HTTPException(status_code=503")
$insertPosition = $content.IndexOf("raise HTTPException(status_code=503, detail=f'Bloomberg Terminal error: {str(e)}')")
if ($insertPosition -gt 0) {
    # Find the next double newline after this position
    $insertPosition = $content.IndexOf("`n`n", $insertPosition) + 2
    
    # Insert the new code
    $newContent = $content.Insert($insertPosition, $newEndpointCode + "`n")
    
    # Save the file
    $newContent | Set-Content 'C:\BloombergAPI\main.py' -NoNewline
    
    Write-Host "✅ Bloomberg reference endpoint added successfully"
    Write-Host "📍 Added after FX rates endpoint"
} else {
