...
with open('/Users/mikaeleage/Projects Container/bloomberg-volatility-component/current_main.py', 'r') as f:
    content = f.read()

# The new reference endpoint with proper logging
new_reference_endpoint = '''@app.post('/api/bloomberg/reference')
...
# Replace the current reference endpoint
pattern = r'@app\.post\(\'/api/bloomberg/reference\'\).*?return \{\s*\'success\': False,\s*\'error\': str\(e\)\s*\}'
content = re.sub(pattern, new_reference_endpoint, content, flags=re.DOTALL)

# Write the updated content
with open('/Users/mikaeleage/Projects Container/bloomberg-volatility-component/updated_main.py', 'w') as f:
    f.write(content)

print("Updated reference endpoint with proper logging")